classdef casadi_block_MPCtry_4 < matlab.System & matlab.system.mixin.Propagates & matlab.system.mixin.SampleTime
    ...
    methods (Access = protected)
        ...
        function [dt1,dt2,dt3,dt4,dt5,dt6,dt7, dt8] = getInputDataTypeImpl(~)
            dt1 = 'double';
            dt2 = 'double';
            dt3 = 'double';
            dt4 = 'double';
        ...
        function [sz1,sz2,sz3,sz4,sz5,sz6,sz7, sz8] = getInputSizeImpl(~)
            sz1 = 1;
            sz2 = 1;
            sz3 = 1;
            sz4 = 1;
        ...
        function [cp1,cp2,cp3,cp4,cp5,cp6,cp7, cp8] = isInputComplexImpl(~)
            cp1 = false;
            cp2 = false;
            cp3 = false;
            cp4 = false;
        ...
        function [fz1,fz2,fz3,fz4,fz5,fz6,fz7,fz8] = isInputFixedSizeImpl(~)
            fz1 = true;
            fz2 = true;
            fz3 = true;
            fz4 = true;
        ...
        function setupImpl(casobj)
            ...
            Q = diag([110;0;0;50;240;240]); % weighing matrices (states) % high speed penalty causes spike in yaw rate at fast segment
            ...
            %casobj.ppx1 = ppx1; casobj.ppy1 = ppy1; 
            %casobj.theta_arr = theta_arr; 
            %casobj.ppx2 = ppx2;
            %casobj.ppy2 = ppy2; casobj.ppx3 = ppx3; casobj.ppy3 = ppy3; 
            %casobj.ppx4 = ppx4; casobj.ppy4 = ppy4; 
            %casobj.ppx5 = ppx5; casobj.ppy5 = ppy5;  
            %casobj.XY1 = XY1; casobj.XY2 = XY2; casobj.XY3 = XY3; casobj.XY4 = XY4; casobj.XY5 = XY5; casobj.XY6 = XY6; casobj.XY7 = XY7; casobj.XY8 = XY8;casobj.XY9 = XY9; casobj.XY10 = XY10;
            ...
            % exit 2 paddig
            %casobj.path(size(casobj.comb_x,2)+1:size(casobj.comb_x,2)+50,1) = casobj.comb_x(1,end);
            %casobj.path(size(casobj.comb_x,2)+1:size(casobj.comb_x,2)+50,2) = linspace(casobj.comb_y(1,end),casobj.comb_y(1,end)+50,50);
            
            % exit 3 padding
            % casobj.path(size(casobj.comb_x,2)+1:size(casobj.comb_x,2)+50,1) = linspace(casobj.comb_x(1,end),casobj.comb_x(1,end)-50,50);
            % casobj.path(size(casobj.comb_x,2)+1:size(casobj.comb_x,2)+50,2) = casobj.comb_y(1,end);
            %plot(casobj.path(:,1), casobj.path(:,2), 'or');
        end
     
        
        function [delta_out,acc_out,break_out,vref,err] = stepImpl(casobj,xd_in,yd_in,yawd_in,yaw_in,x_in,y_in,t, ref_path)
            casobj.path = ref_path;  
            err=0;
             err_ve=0;
            ...
            ex = 0; 
            if ref_path(end,1) > 0 && ref_path(end,2) < 0 
                ex = 1;
            else if ref_path(end,1) > 0 && ref_path(end,2) > 0
                    ex = 2;
                else if ref_path(end,1) < 0 && ref_path(end,2) > 0 
                        ex = 3; 
                    else if ref_path(end,1) < 0 && ref_path(end,2) < 0 
           ...
           path_ref = zeros(casobj.N,2);
            if ex == 3 
                if (y_in >= 0) || x_in < 0
                    ...
                    path_ref(1:len_t,1) = tx;
                    path_ref(1:len_t,2) = ty;
                    if size(t_ind,1) < casobj.N
                        path_ref(len_t+1:end,1) = path_ref(len_t,1);
                        path_ref(len_t+1:end,2) = path_ref(len_t,2);
                    end                    
                end                
                if y_in < 0 && x_in>0
                    ...
                    if size(next_N_ind,1) < casobj.N
                        ...
                        path_ref(len_N+1:end,1) = path_ref(len_N,1);
                        path_ref(len_N+1:end,2) = path_ref(len_N,2);
                    else
